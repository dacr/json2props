...
object Xml {
  ...
  def toJson(xml: NodeSeq): JValue = {
    ...
    //def isArray(nodeNames: Seq[String]) = nodeNames.size != 1 && nodeNames.toList.distinct.size == 1  // ************** Specific DCR changes
    def isArray(nodeNames: Seq[String]) = false // ************** Specific DCR changes
    ...
    def toJValue(x: XElem): JValue = x match {
      ...
      case XLeaf((name, value), attrs) => (value, attrs) match {
        ...
        //case (XValue(""), xs) => JObject(mkFields(xs))   // ************** Specific DCR changes - patch coming from https://github.com/json4s/json4s/issues/24
        case (XValue(""), xs) => JObject((name, JObject(mkFields(xs))))  // ************** Specific DCR changes - patch coming from https://github.com/json4s/json4s/issues/24
...
//      case JLong(x) => new XmlElem(name, x.toString)
  ...
  private class XmlNode(name: String, children: Seq[Node]) extends Elem(null, name, xml.Null, TopScope, children.isEmpty, children :_*)
